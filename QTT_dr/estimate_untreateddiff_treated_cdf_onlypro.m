function [fval] =estimate_untreateddiff_treated_cdf_onlypro(gammahat,x1,x2,x3,x4,D,DY2,prob,delta)
...
for p=1:size(DY2)
    f_element(p,1) =((((1-D(p,1))/prob)*(L(p,1)/(1-L(p,1))))*indicator_DY2(p,1))./mean(((1-D)/prob).*(L ./(1-L)),1);
end
